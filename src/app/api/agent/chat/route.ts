...
/** Conservative token estimate: ~1 token per 2 chars for Korean-heavy text in LLaMA 3.1 */
...
function buildSystemPrompt(places: DbPlace[], availableTokens: number): string {
  ...
  // Include as many places as the token budget allows
  const included: DbPlace[] = [];
  let usedTokens = 20; // overhead for section headers and grouping

  for (const p of sorted) {
    const line = formatPlace(p);
  ...
  const groups = groupPlacesByCuisine(included);
  ...
  const truncationNote =
    included.length < places.length
      ? `\n참고: 토큰 제한으로 ${places.length}개 중 ${included.length}개만 표시됩니다. 목록에 없는 곳을 물어보면 보이는 범위 내에서만 추천 가능하다고 안내하세요.`
      : "";

  return `${SYSTEM_PROMPT_TEMPLATE}${truncationNote}

## 저장된 맛집 (${included.length}개)
${structuredList}`;
}
